...
import UserSidebar from '../components/UserSidebar'; // Fixed import path for UserSidebar
...
const HomePage = () => {
  ...
  return (
    <div className="bg-gray-100 min-h-screen flex">
      {/* Sidebar */}
      <UserSidebar /> {/* Added the UserSidebar component */}

      <div className="flex-1 p-6">
        <Header />

        {/* MyProfile added below header */}
        <MyProfile />

        {/* Search Bar */}
        <div className="flex justify-between items-center mb-6 flex-wrap bg-white p-4 shadow-md rounded-md">
          <input
            type="text"
            placeholder="Search incidents by title..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="bg-gray-50 border border-gray-300 text-gray-900 py-2 px-4 rounded-md w-full sm:w-2/3 md:w-1/2 lg:w-1/3 focus:outline-none focus:border-indigo-500"
          />
        </div>
        ...
        <div className="flex justify-between items-center mb-6 flex-wrap bg-white p-4 shadow-md rounded-md">
          <select
            ...
            className="bg-gray-50 border border-gray-300 text-gray-900 py-2 px-4 rounded-md w-full sm:w-1/3 focus:outline-none focus:border-indigo-500"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredIncidents.map((incident) => (
            <div key={incident.id} className="bg-white p-6 rounded-md shadow-lg transform hover:scale-105 transition-transform duration-300">
              <h3 className="text-xl font-semibold mb-2 text-indigo-600">{incident.title}</h3>
              ...
              {/* Media Gallery */}
              <Carousel showThumbs={false} dynamicHeight={true} className="mb-4 rounded-md shadow-sm overflow-hidden">
        ...
        {/* CreateIncident component added here */}
        <div className="mt-10">
